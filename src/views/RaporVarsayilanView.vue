...
export default {
  ...
  methods: {
    getAuthorsWithFilter() {
      ...
      axios.get(`//burdanoku.com/wp-json/wp/v2/users?per_page=100&before=${end}&after=${start}`)
        .then(response => {
          response.data.forEach(author => {
            axios.get(`//burdanoku.com/wp-json/wp/v2/posts?author=${author.id}&before=${end}&after=${start}`)
              .then(res => {
