...
export default {
  ...
  methods: {
    getAuthorsWithFilter() {
      ...
      axios.get(`//linehaber.com.tr/wp-json/wp/v2/users?per_page=100&before=${end}&after=${start}`)
        .then(response => {
          response.data.forEach(author => {
            axios.get(`//linehaber.com.tr/wp-json/wp/v2/posts?author=${author.id}&before=${end}&after=${start}`)
              .then(res => {
